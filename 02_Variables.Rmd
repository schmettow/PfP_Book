...
When you took a closer look at the Stroop task program in chapter 1 [Introduction], you encountered components of a computer program called *variables*
...
In the examples above, the *printed* outputs `4`, `5.0` and `Hello, World!` are *values*. Values can thus be numbers, but also words, or even whole sentences. In the interior of a computer program, `4` and `Hello, World` serve the same purpose, they are both arbitrary values. Values are classified into different *classes*, or *data types*: `4` is a so-called *integer* and `"Hello, World"` is a
...
The order in which the variable and a value that is to be assigned to a variable are placed
...
specialized types. You already encountered one such a specialized data type: *dictionaries*. Literally any computer program contains values of at least the basic types and thus, it seems to be reasonable to say a word or two about them. 
