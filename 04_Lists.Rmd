...
A special type of list is the so-called *empty list*. An empty list is a list with no elements. Empty lists are denoted with `[]`. Lists can even be *nested* into one another to form lists of lists.
...
Lists are *ordered* collections of values. However, a list's internal order has nothing to do with how humans may be inclinded to order list elements. Humans may be inclinded to order a list `a` with three elements `"A", "B",` and `"C"` according to their alphabetic order `a = ["A", "B", "C"]`. However, for lists, the value of an element is irrelevant for its position in the list. Lists are not *ordered* because they grasp some underlying semnatics about their elements and rank their elements accordingly. In fact, element values are arbitrary for a list. Instead, lists are called *ordered* because they allocate a certain position in memory to each element. Each list element occupies a unique position in memory. Elements of a list are accessed by querying the list at a specified position, a so-called *index*. Accessing a list at a specified index, the value located at that position in memory is returned. The *index operator* `[]` is used to access list elements at specified indices.
...
```
Because lists are mutable, you can sequentially fill them with elements, step by step. This is very handy for storing values you generate throughout your program. There are several ways of adding elements to a list.
...
The *append* method adds the specified element at the end of the list. You can use the *extend* method to add the values of a second list (or other type of collection) to your list.
...
In the above examples, when `a` is **extended** by `b`, the elements stored in `b` are sequentially appended to `a`, each of them constituting a new element in `a`. In contrast, the whole list `b` is regarded as an element of `a` when `b` is **appended** to `a`.

Another way of mutating a list is by using the index operator. You can use the index operator to change the value of an element in a list.
...
Note that the statement `a[0:2] = d` mutates both elements, `1` *and* `"a"` into a single new item `"d"`. This means that it did not only change some elements of `a`, but also `a`'s total number of elements!
...
###A word about class specific methods
By now, you have seen a few examples of class specific *methods*, such as `append()` for lists or `join()` for strings. Even though a detailed explanation of *methods* (also called *functions*) is postponed to Chapter \@ref(functions), a quick introduction to objects, classes, and class specific methods will help you make the best use of your lists and their built-in functions. For now, you can see functions as pieces of reusable code.

When you create a variable `a` and assign a value to it, say `[4,5]`, you can think of it as creating an *object* (i.e. an instance) of the Python *class* *list*. The class *list* has been programmed by the developers of the Python language, same as any other built-in Python class. Classes are blueprints for objects and only come into existence when you create an instance of that class. A class has built-in *methods* that can be used by *any* instance of the class. This is very useful, because you can use a wide range of useful code snippets specifically designed for (in this case) lists, simply because your lists inherit these methods from a overarching class definition. An example is the `append()` method for the `list` class.
...
The take-home message for now is: make use of built-in class functions as much as you can! They provide a tremendous amount of functionality which you are likely going to need and want, written in neat code which makes them very efficient too!
