...
In contrast to Strings, however, the elements of list can be of *any* type. Ordered collections such as strings and lists are also called *sequences*.

In ordered collections, the order of elements is significant. Each element occupies a unique position in a list. Elements of a list are accessed by querying the list at a specified position, at a so-called *index*,  and returning the value(s) located at that position. Another type of operator, the *index operator* `[]` is used to access list elements.
