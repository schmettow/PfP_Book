...
By taking a close look at how the Stroop Task program transits from one state to another, the transition table can be filled in. For your interest, the changing state part of the Stroop Task program is displayed below, together with the completed transition table for the Stroop Task program.
...
Transition tables are also useful when you conceive your own program. They serve as the interaction skeleton to which you gradually add functionality as you develop your program.
...
3. Exclusively using `if clauses` in a chain of conditionals. This is a tricky one because the consequences may not immediately be apparent and no error is necessarily thrown. Still, you program somehow does not do what you expect it to do. In truth, exclusively using `if clauses` is not wrong; but chances are high that you intended to use a chain of `if/elif clauses` instead. Let's take a look at the following two code snippets to illustrate the difference between a chain of `if clauses` and a chain of `if/elif clauses`.
