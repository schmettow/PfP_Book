...
At times it may be confusing whether either a `for` or a `while` loop will suit your needs best. If you are unsure which kind of loop you should use, ask yourself the following questions:

* Do I know in advance how many times I need the loop body to be executed? Problems like "searching a collection for a specified element" or "executing code until the desired number of trials is reached" suggest that a `for` loop will suit your needs best.
